...
public class ClashDetection : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        print($"Distance between start to end for BLUE one: {InformedRRTStar.get_path_len(path):F3} m");
        ...
        path_Reverse_1 = priorityObject.GetComponent<GameManager_1>().Getpath();
        b_1 = priorityObject.GetComponent<GameManager_1>().Get_b();
        m_1 = priorityObject.GetComponent<GameManager_1>().Get_m();
        ...
        {
            EditorUtility.DisplayDialog("warning!", "There is not a intersection point in the path of two objects.", "ok");
            Warning = true;
        }

        angle_Intersection = Vector3.Angle(FindVector(new Vector3(x_Intersection, 0, y_Intersection)), priorityObject.GetComponent<GameManager_1>().FindVector(new Vector3(x_Intersection, 0, y_Intersection)));

        Vector3 direction_Intersection = FindVector(new Vector3(x_Intersection, 0, y_Intersection)).normalized;
        Vector3 direction_Intersection_1 = priorityObject.GetComponent<GameManager_1>().FindVector(new Vector3(x_Intersection, 0, y_Intersection)).normalized;
        ...
        firstIndex_1 = priorityObject.GetComponent<GameManager_1>().FindFirstIndex(intersectionPoint);
        endIndex_1 = priorityObject.GetComponent<GameManager_1>().FindEndIndex(intersectionPoint);
        ...
        if (Warning == false)
        {
            ...
            // [1]
            if (length <= Vector3.Distance(intersectionPoint, FindFirstPoint(intersectionPoint)) && length_1 <= Vector3.Distance(intersectionPoint, priorityObject.GetComponent<GameManager_1>().FindFirstPoint(intersectionPoint)))
            {
                if (length <= Vector3.Distance(intersectionPoint, FindEndPoint(intersectionPoint)) && length_1 <= Vector3.Distance(intersectionPoint, priorityObject.GetComponent<GameManager_1>().FindEndPoint(intersectionPoint)))
                {
                    print("1-5");
                ...
                else if (length > Vector3.Distance(intersectionPoint, FindEndPoint(intersectionPoint)) && length_1 <= Vector3.Distance(intersectionPoint, priorityObject.GetComponent<GameManager_1>().FindEndPoint(intersectionPoint)))
                {
                    print("1-6");
                    ...
                    if (endIndex < path_Reverse.Count - 1)
                    {
                        ...
                        angle_new = Vector3.Angle(vector_new, priorityObject.GetComponent<GameManager_1>().FindVector(new Vector3(x_Intersection, 0, y_Intersection)));
            ...
            // [2]
            else if (length > Vector3.Distance(intersectionPoint, FindFirstPoint(intersectionPoint)) && length_1 <= Vector3.Distance(intersectionPoint, priorityObject.GetComponent<GameManager_1>().FindFirstPoint(intersectionPoint)))
            {
                if (length <= Vector3.Distance(intersectionPoint, FindEndPoint(intersectionPoint)) && length_1 <= Vector3.Distance(intersectionPoint, priorityObject.GetComponent<GameManager_1>().FindEndPoint(intersectionPoint)))
                {
                    print("2-5");
                ...
                else if (length > Vector3.Distance(intersectionPoint, FindEndPoint(intersectionPoint)) && length_1 <= Vector3.Distance(intersectionPoint, priorityObject.GetComponent<GameManager_1>().FindEndPoint(intersectionPoint)))
                {
                    ...
                    if (firstIndex > 0 && endIndex < path_Reverse.Count - 1)
                    {
                        ...
                        angle_new_second = Vector3.Angle(vector_new_second, priorityObject.GetComponent<GameManager_1>().FindVector(new Vector3(x_Intersection, 0, y_Intersection)));
                ...
                else if (length <= Vector3.Distance(intersectionPoint, FindEndPoint(intersectionPoint)) && length_1 > Vector3.Distance(intersectionPoint, priorityObject.GetComponent<GameManager_1>().FindEndPoint(intersectionPoint)))
                {
                    ...
                    else
                    {
                        ...
                        if (firstIndex > 0 && firstIndex_1 > 0 && endIndex_1 < path_Reverse_1.Count - 1)
                        {
                            ...
                            angle_new_third = Vector3.Angle(vector_new_third, priorityObject.GetComponent<GameManager_1>().FindVector(new Vector3(x_Intersection, 0, y_Intersection)));
            ...
            s1 = priorityObject.GetComponent<GameManager_1>().PointToTime(start_1_point);
            s2 = priorityObject.GetComponent<GameManager_1>().PointToTime(exit_1_point);
    ...
    void FixedUpdate()
    {
        ...
        if (!finish)
        {
            ...
            else if (flag1 == true && t2 < s1 && t2 < s2)
            {
                ...
                if (timer >= s2 && timer < s2 + 0.04f)
                {
                    clash = false;
                }
            }

            else if (flag2 == true && t1 > s1 && t1 > s2 && showMsg2 == true)
            {
                EditorUtility.DisplayDialog("Warning!", "There is not a way to solve this solution, so please change the starting and ending points of objects!", "ok");
                showMsg2 = false;
            }

            else if (t1 > s1 && t1 < s2 && t2 > s2)
            {
        ...
        if (showMsg == true && priorityObject.GetComponent<GameManager_1>().ShowmMessage() == true && Warning == false && PlusMsg == true)
        {
            EditorUtility.DisplayDialog("Result", $"Time for Blue Cube: {reachTime:F3} sec\n" +
                                                  $"Time for Red Cube: {priorityObject.GetComponent<GameManager_1>().ReachTime():F3} sec\n " +
                                                  $"\nThe Blue cube must stop at time = {t1:F3} sec and start moving at time = {s2:F3} sec \n" +
                                                  $"\nintersection point: ({intersectionPoint.x:F3}, {intersectionPoint.y:F3}, {intersectionPoint.z:F3})", "ok");

            showMsg = false;
        }

        else if (showMsg == true && priorityObject.GetComponent<GameManager_1>().ShowmMessage() == true && Warning == false && PlusMsg == false)
        {
            EditorUtility.DisplayDialog("Result", $"Time for Blue Cube: {reachTime:F3} sec\n" +
                                                  $"Time for Red Cube: {priorityObject.GetComponent<GameManager_1>().ReachTime():F3} sec\n " +
                                                  $"\nintersection point: ({intersectionPoint.x:F3}, {intersectionPoint.y:F3}, {intersectionPoint.z:F3})", "ok");
        ...
        else if (showMsg == true && priorityObject.GetComponent<GameManager_1>().ShowmMessage() == true && Warning == true)
        {
            EditorUtility.DisplayDialog("Result", $"Time for Blue Cube: {reachTime:F3} sec\n" +
                                                  $"Time for Red Cube: {priorityObject.GetComponent<GameManager_1>().ReachTime():F3} sec\n ", "ok");
