...
namespace Assets.Scripts
{
    public class InformedRRTStar
    {
        ...
        public InformedRRTStar(double[] start_pos, double[] goal_pos, double[] diameter, double[,] obstacleList, double[] randArea,
            double expandDis = 0.5, int goalSampleRate = 10, int maxIter = 1000, double safeDistance = 0.1f)
        {
        ...
        public bool check_segment_collision(double x1, double y1, double x2, double y2)
        {
            for (int i = 0; i < obstacleList.Length / 3; i++)
            {
                ...
                if (Math.Sqrt(dd) <= obstacleList[i, 2] + Math.Sqrt(Math.Pow(diameter[0], 2) + Math.Pow(diameter[1], 2)) / 2.0 + safeDistance)
                {
                    return false;
                }
            }
