...
public class Returning_1 : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        start_Pos_x = GetComponent<Hauling_1>().goal_Pos_x;
        start_Pos_z = GetComponent<Hauling_1>().goal_Pos_z;
        ...
        string str = "Optimal Path from Dumping Point to Starting Point for RED one : ";
        ...
        print($"Distance between Dumping Point to Starting Point for RED one: {InformedRRTStar.get_path_len(path) * 5:F3} m");
        print("***********************************************************");


        //Draw Line between nodes
        lineRenderer = GetComponent<LineRenderer>();
        lineRenderer.positionCount = path.Count + GetComponent<Hauling_1>().Getpath_line().Count + GetComponent<Loading_1>().Getpath_line().Count - 2;
        ...
        for (int i = 1; i < GetComponent<Hauling_1>().Getpath_line().Count; i++)
        {
            lineRenderer.SetPosition(i + path.Count - 1, new Vector3(Convert.ToSingle(GetComponent<Hauling_1>().Getpath_line()[i].x), 0.5f, Convert.ToSingle(GetComponent<Hauling_1>().Getpath_line()[i].y)));
        }

        for (int i = 1; i < GetComponent<Loading_1>().Getpath_line().Count; i++)
        {
            lineRenderer.SetPosition(i + path.Count + GetComponent<Hauling_1>().Getpath_line().Count - 2, new Vector3(Convert.ToSingle(GetComponent<Loading_1>().Getpath_line()[i].x), 0.5f, Convert.ToSingle(GetComponent<Loading_1>().Getpath_line()[i].y)));
        }
    ...
    void FixedUpdate()
    {
        if (GetComponent<Hauling_1>().FinishiMessage() == true)
        {
            ...
            //follow path
            if (!finish && timer_1 > smallTruckDumping)
            {
